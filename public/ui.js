	...
	imageCompression.prototype = {
		...
		templateCompleted: function(data) {
			var templateCompleted = '<h2>Completed</h2>';
				...
				templateCompleted += '</ul>';
				templateCompleted += '<a href="/uploads/' + data.createdKey + '_images.zip" download="' + data.createdKey + '_images.zip" target="_blank">completed zipfile ' + data.createdKey + '_images.zip</a>';
				templateCompleted += '';
			...
			if (files.length > 0) {
				...
				$.ajax({
					...
					success: function(data){
						console.log(data);
						_this.$completedArea.addClass('active').empty().append(_this.templateCompleted(data))
						_this.$dimmed.hide().find('.msg .txt').text('image uploading')
										.end().find('.percentBar span').width(0);
						...
						xhr.upload.addEventListener('progress', function(evt) {
							if (evt.lengthComputable) {
								...
								if (percentComplete === 100) {
									_this.$dimmed.find('.msg .txt').text('doing compressed')
												.end().find('.percent').text('');
								}
							}
						}, false);
		...
		makeList: function(files){
			...
			if (files.length !== undefined) {
				...
				_this.$uploadList.addClass('active').empty().append(list);
			} else {
				// type drag drop
