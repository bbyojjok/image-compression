...
route.get('/history/:page', (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query('SELECT count(*) as totalCount from completed', (err, results) => {
      ...
      const page =
        req.params.page > totalPage || req.params.page <= 0 ? 1 : parseInt(req.params.page);
      ...
      const sql =
        'SELECT id,ip,browser,created_key,compressibility,zip_url,before_filesize,after_filesize,DATE_FORMAT(created, "%Y/%m/%d %T") created FROM completed ORDER BY id DESC LIMIT ?, ?';
...
route.post('/api/upload', (req, res) => {
  ...
  form.on('end', (fields, files) => {
    formEnd(form.openedFiles)
      ...
      .then(result => {
        ...
        const sql =
          'INSERT INTO completed (ip, browser, created_key, compressibility, zip_url, before_filesize, after_filesize, created) VALUES(?, ?, ?, ?, ?, ?, ?, NOW())';
        const params = [
          ip,
          ua,
          resultFiles.createdKey,
          resultFiles.compressibility,
          'uploads/' + createdKey + '_images.zip',
          resultFiles.beforeSizeFormatBytes,
          resultFiles.afterSizeFormatBytes
        ];
        pool.getConnection((err, connection) => {
          if (err) {
            connection.release();
  ...
  function getFilesize(files) {
    ...
    return new Promise((resolve, reject) => {
      ...
      resultFiles.compressibility =
        (((resultFiles.beforeSize - resultFiles.afterSize) / resultFiles.beforeSize) * 100).toFixed(
          2
        ) + ' %';
  ...
  function zipFileAdd() {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < resultFiles.fileList.length; i++) {
        ...
        zip.file(
          item.fileName,
          fs.readFileSync(`uploads/${createdKey}_images/compress/${item.fileName}`),
          { base64: true }
        );
      }
      zip
